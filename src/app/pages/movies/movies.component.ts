...
export class MoviesComponent implements OnInit {
  ...
  loadAllCategories() {
    ...
    const requests = [
      this.svc.getPopular(1),
      this.svc.getTopRated(1),
      this.svc.getUpcoming(1),
      this.svc.getNowPlaying(1)
    ];

    Promise.all(requests).then(responses => {
      this.movieCategories[0].movies = responses[0].results.slice(0, 20);
      this.movieCategories[1].movies = responses[1].results.slice(0, 20);
      this.movieCategories[2].movies = responses[2].results.slice(0, 20);
      this.movieCategories[3].movies = responses[3].results.slice(0, 20);
      
      // Set featured movie from popular movies
      if (responses[0].results.length > 0) {
        this.featuredMovie = responses[0].results[0];
      }
